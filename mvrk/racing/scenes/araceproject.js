    ...
    class UniversalCameraSystem extends BABYLON.ScriptComponent {
        ...
        async startCameraSystemState() {
            ...
            if (this.m_cameraRig != null) {
                ...
                if (this.m_cameraRig.inputs != null) {
                    //this.m_cameraRig.inputs.attached.mouse.touchEnabled = !this.cameraController.disableTouches;
                }
                ...
                else if (this.cameraType === 3) { // Free Move Camera
                    const freeCameraMaster = this.m_cameraRig;
                    this.m_cameraRig = new BABYLON.FreeCamera(freeCameraMaster.name, new BABYLON.Vector3(0, 0, 0), this.scene);
                    this.m_cameraRig.parent = freeCameraMaster.parent;
                    this.m_cameraRig.position = freeCameraMaster.position.clone();
                    if (freeCameraMaster.rotationQuaternion != null) {
                        this.m_cameraRig.rotationQuaternion = freeCameraMaster.rotationQuaternion.clone();
                    }
                    else if (freeCameraMaster.rotation != null) {
                        this.m_cameraRig.rotation = freeCameraMaster.rotation.clone();
                    }
                    this.scene.switchActiveCamera(this.m_cameraRig, false); // Note: Do Not Auto Attach
                    freeCameraMaster.dispose();
                }
                else if (this.cameraType === 4) { // Arc Rotate Camera
                    const targetVector = BABYLON.Utilities.ParseVector3(this.arcRotateConfig.target);
                    const arcCameraMaster = this.m_cameraRig;
                    this.m_cameraRig = new BABYLON.ArcRotateCamera(arcCameraMaster.name, 0, 0, 0, targetVector, this.scene);
                    this.m_cameraRig.parent = null; // Note: Always Reset Parent
                    this.m_cameraRig.setPosition(new BABYLON.Vector3(this.arcRotateConfig.alpha, this.arcRotateConfig.beta, this.arcRotateConfig.radius));
                    this.m_cameraRig.wheelPrecision = this.arcRotateConfig.wheel;
                    this.scene.switchActiveCamera(this.m_cameraRig, this.cameraController.attachControl);
                    arcCameraMaster.dispose();
                }
                // ..
                // Override Default Attach Control
                // ..
                if (this.cameraType !== 4) { // Note: Do Not Re Position Arc Rotate Camera Here
    ...
    class StandardCarController extends BABYLON.ScriptComponent {
        ...
        drive(throttle, steering, braking, drifting, assisted = true, autopilot = false) {
            ...
            if (this.FRONT_LEFT >= 0 && this.FRONT_RIGHT >= 0 && this.BACK_LEFT >= 0 && this.BACK_RIGHT >= 0) {
                ...
                // const msg:string = ("Car Speed: " + this.americanSpeed.toFixed(1) + " --> Gear Index: " + this.gearIndex.toFixed(0) + " --> Engine RPM: " + this.engineRPM.toFixed(0) + " --> Speed Throttle: " + throttleSpeed.toFixed(0));
        ...
        getVehicleEngineTorque(rpm) {
            let result = 0;
            if (this.engineTorqueCurve != null) {
                result = (BABYLON.Utilities.SampleAnimationFloat(this.engineTorqueCurve, BABYLON.Scalar.Clamp(rpm, StandardCarController.MIN_RPM, StandardCarController.MAX_RPM)) * this.powerCoefficient);
            }
            return result;
        }
...
(function (BABYLON) {
    ...
    class AnimationState extends BABYLON.ScriptComponent {
        ...
        lateStateMachine(deltaTime = null) {
        ...
        setupWheelInformation(defaultAngularFactor = null) {
            ...
            if (this.m_vehicle != null && this.m_vehicleColliders != null && this.m_vehicleColliders.length > 0) {
                ...
                if (this.m_vehicle.getNumWheels() === this.m_vehicleColliders.length) {
                    for (index = 0; index < this.m_vehicleColliders.length; index++) {
                        const wheel = this.m_vehicleColliders[index];
                        const defaultForce = (wheel.totalsuspensionforces != null) ? wheel.totalsuspensionforces : 1000000;
                        const defaultTravel = (wheel.suspensiontravelcm != null) ? wheel.suspensiontravelcm : 25;
                        const defaultRolling = (wheel.rollinfluence != null) ? wheel.rollinfluence : 0.2;
                        const defaultFriction = (wheel.frictionslip != null) ? wheel.frictionslip : 10;
                        const suspensionStiffness = (wheel.suspensionstiffness != null) ? wheel.suspensionstiffness : 50;
                        const suspensionCompression = (wheel.dampingcompression != null) ? wheel.dampingcompression : 3.5;
