/////////////////////////////////////////////////////////////////////////////////////////////
// Mackey Kinard - GLTF Canvas Tools Interface
/////////////////////////////////////////////////////////////////////////////////////////////
(function () {
    ...
    function translateAnimation(data, resources) {
        ...
        //////////////////////////////////////////////////////
        // Mackey Kinard - HACK: Support Animation Meta Data
        //////////////////////////////////////////////////////
        if (data.hasOwnProperty('extras') && data.extras.hasOwnProperty('metadata')) {
            clip.metadata = data.extras.metadata;
        }
        //////////////////////////////////////////////////////
    ...
    function translateMaterial(data, resources) {
        ...
        if (data.hasOwnProperty('extensions') && data.extensions.hasOwnProperty('KHR_materials_pbrSpecularGlossiness')) {
            ...
            ////////////////////////////////////////////////////////////////
            // Mackey Kinard - Shininess Is Inverted Here
            ////////////////////////////////////////////////////////////////
            // Note: 0 Is Totally Shinny And 100 Is No Shininess
            ////////////////////////////////////////////////////////////////
            var shininessBuffer = 0;
            if (pbrData.hasOwnProperty('roughnessFactor')) {
                var roughnessBuffer = 1.0 - pbrData.roughnessFactor 
                shininessBuffer = 100 * roughnessBuffer;
            } else {
                shininessBuffer = 0;
            }
            material.shininess = (100 - shininessBuffer);
        ...
        if (data.hasOwnProperty('commonConstant') && data.commonConstant.hasOwnProperty('lightmapTexture')) {
            ...
            // TODO: Process Shadow Mask Texture
        }
        ...
        // Mackey Kinard - Note: Canvas Tools Library Hooking
        if (resources != null && resources.app != null) {
            if (typeof CanvasTools !== undefined && typeof CanvasTools.TranslateMaterial !== undefined) {
                CanvasTools.TranslateMaterial(resources.app, material, data, resources);
            }
        }
    ...
    function translateMesh(data, resources) {
        ...
        //////////////////////////////////////////////////////
        // Mackey Kinard - HACK: Support Inital Morph Weights
        //////////////////////////////////////////////////////
        ...
        data.primitives.forEach(function (primitive) {
            ...
            /////////////////////////////////////////////
            // Mackey Kinard - HACK: Store Morph Weights
            /////////////////////////////////////////////
    ...
    // Mackey Kinard - GLTF Canvas Tools Interface
    /////////////////////////////////////////////////////////////////////////////////////////////
    function translateNodeGraph(data, resources) {
        var graph = new pc.GraphNode();

        if (data.hasOwnProperty('name')) {
            graph.name = data.name;
        } else {
            graph.name = 'Node ' + resources.nodeCounter;
        }
        ...
        if (data.hasOwnProperty('matrix')) {
            ...
            graph.setLocalPosition(tempVec);
            tempMat.getEulerAngles(tempVec);
            graph.setLocalEulerAngles(tempVec);
            tempMat.getScale(tempVec);
            graph.setLocalScale(tempVec);
        }

        if (data.hasOwnProperty('rotation')) {
            var r = data.rotation;
            graph.setLocalRotation(r[0], r[1], r[2], r[3]);
        }

        if (data.hasOwnProperty('translation')) {
            var t = data.translation;
            graph.setLocalPosition(t[0], t[1], t[2]);
        }

        if (data.hasOwnProperty('scale')) {
            var s = data.scale;
            graph.setLocalScale(s[0], s[1], s[2]);
        }
    ...
    function translateNodeEntity(data, resources) {
        ...
        // Mackey Kinard - Note: Default Animation Component
        entity.animations = null;

        // Mackey Kinard - Note: Canvas Tools Library Hooking
        if (typeof CanvasTools !== undefined && typeof CanvasTools.TranslateNode !== undefined) {
            CanvasTools.TranslateNode(resources.app, entity, data, resources);
        }
    ...
    function translateTexture(data, resources) {
        ...
        // Mackey Kinard - GLTF Canvas Tools Interface
        ////////////////////////////////////////////////////////////////////
        texture.anisotropy = 2; // Set Default Ansiotropy Level 
        if (data.hasOwnProperty('sampler')) {
            ...
            // Mackey Kinard - GLTF Canvas Tools Interface
            ////////////////////////////////////////////////////////////////////
            if (sampler.hasOwnProperty('extras') && sampler.extras.metadata != null) {
                if (sampler.extras.metadata.hasOwnProperty('anisolevel')) {
                    texture.anisotropy = sampler.extras.metadata.anisolevel;
                }
            }
        }
        ...
        gltf.nodes.forEach(function (node, idx) {
            if (node.hasOwnProperty('mesh')) {
                ...
                for (var i = 0; i < meshGroup.length; i++) {
                        ...
                        if (meshGroup[i].hasOwnProperty('_morphWeights')) {
                            ...
                            if (morphWeights != null && morphWeights.length > 0) {
                                for (let mi = 0; mi < morphWeights.length; mi++) {
                                    try { 
                                        morphInstance.setWeight(mi, morphWeights[mi]);
                                    } catch(err) {
                    ...
                    //////////////////////////////////////////////////////////////
                    // Mackey Kinard - HACK: Delete Morph Weight Buffer
                    //////////////////////////////////////////////////////////////
                    if (meshGroup[i].hasOwnProperty('_morphWeights')) {
    ...
    // Mackey Kinard - GLTF Canvas Tools Interface
    ////////////////////////////////////////////////////////////////////
    function createSceneModels(resources) {
        resources.gltf.nodes.forEach(function (node, idx) {
            if (node.hasOwnProperty('mesh')) {
                ...
                if (meshGroup.length > 0) {
                    ...
                    // Mackey Kinard - Note: Canvas Tools Library Hooking
                    if (typeof CanvasTools !== undefined && typeof CanvasTools.TranslateModel !== undefined) {
                        CanvasTools.TranslateModel(resources.app, entity, resources);
                    }
                }
            }
        });
    ...
    function getResourceEntity(resources, id) {
        var entity = null;
        if (resources.nodes != null) {
            for (index = 0; index < resources.nodes.length; index++) {
                var element = resources.nodes[index];
                if (element.hasOwnProperty('metadata') && element.metadata != null) {
                    if (element.metadata.hasOwnProperty('id') != null) {
    ...
    function parseAnimations(resources) {
        if (resources.animations != null) {
            resources.animations.forEach((anim) => {
                if (anim.hasOwnProperty('metadata')) {
                    var owner = anim.metadata.hasOwnProperty('owner') ? anim.metadata.owner : null;
                    if (owner != null) {
                        var ownerEntity = getResourceEntity(resources, owner);
                        if (ownerEntity != null) {
                            if (ownerEntity.animations == null) ownerEntity.animations = new AnimationComponent();
                            ownerEntity.animations.addClip(anim);
                        }
                    }
                    var target = anim.metadata.hasOwnProperty('target') ? anim.metadata.target : null;
                    if (target != null) {
                        var targetEntity = getResourceEntity(resources, target);
                        if (targetEntity != null) {
                            for(var c = 0; c < anim.animCurves.length; c++) {
                                var curve = anim.animCurves[c];
                                if (curve.animTargets[0].targetNode === 'model')
                                    curve.animTargets[0].targetNode = targetEntity;
                            }
                        }
                    }
                }
    ...
    function loadGltf(gltf, device, success, options) {
        ...
        // Mackey Kinard - GLTF Canvas Tools Interface
        ////////////////////////////////////////////////////////////////////
        var appid = options ? options.appid : undefined;
        ...
        var sceneFile = (options && options.hasOwnProperty('sceneFile')) ? options.sceneFile : undefined;
        ...
        // Note: Mackey Kinard - Setup Primary Resoueces App Properties
        var resources = {
            app: pc.Application.getApplication(appid),
            basePath: basePath,
            sceneFile: sceneFile,
            buffers: buffers,
            device: device,
        ...
        // Note: Mackey Kinard - Fix Default Material To Pass Resources
        resources.defaultMaterial = translateMaterial({}, resources);
        ...
        // Note: Mackey Kinard - Use Parse Buffers To Bootstrap Loading
        var parseBuffers = function () {
            ...
            parse('images', translateImage, resources, function () {
                ...
                parse('nodes', (sceneFile === true) ? translateNodeEntity : translateNodeGraph, resources);
                parse('skins', translateSkin, resources);
                parse('animations', translateAnimation, resources);

                buildHierarchy(resources);
                parseAnimations(resources);

                if (sceneFile === true) {
                    createSceneModels(resources);
                    success(getRoots(resources), resources.textures, resources.animations);
                } else {
        ...
        // Mackey Kinard - Note: Canvas Tools Library Hooking
        if (sceneFile === true && typeof CanvasTools !== undefined && typeof CanvasTools.TranslateScene !== undefined) {
            CanvasTools.TranslateScene(resources.app, resources, function() {
                loadBuffers(resources, parseBuffers);
            });
        } else {
            loadBuffers(resources, parseBuffers);
        }
